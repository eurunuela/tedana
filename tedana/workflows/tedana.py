...
def tedana_workflow(
    ...
    stages = [
        ...
        ("t2s_estimation", run_t2s_estimation),
        ("echo_combination", run_echo_combination),
        ("pca_decomposition", run_pca_decomposition),
        ("ica_decomposition", run_ica_decomposition),
        ("metrics_classification", run_metrics_and_classification),
        ("component_processing", run_component_processing),
    ...
    for stage_name, stage_fn in stages:
        LGR.debug(f"Running stage: {stage_name}")
        result = stage_fn(config, state)
        ...
        if result.skipped:
            LGR.info(f"Stage '{stage_name}' skipped: {result.skip_reason}")

        for warning in result.warnings:
            LGR.warning(f"[{stage_name}] {warning}")
